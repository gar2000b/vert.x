...
public class SIWSResource {
	...
	public Response sendSINRequest(@PathParam("requestNo") String requestNo,
		...
		try {
			...
			System.out.println("* Fetched customer with SIN of: "
					+ customer.getSIN());
		} catch (Exception e) {
	...
	private Customer fetchCustomer(String customerId) throws Exception {
		...
		WebResource webResource = client.resource("http://"
				+ SIWSApplication.CS_ADDRESS + "/cs/v1/getCustomer/"
				+ customerId);
	...
	private SINRequestResponse insertSINRequest(String requestNo, String customerId) {
		SINRequestResponse sinRequestResponse = new SINRequestResponse(
				requestNo, customerId);
		System.out.println("** ObjectId before insert = " + sinRequestResponse.getId());
		MorphiaAppDriver.saveData(sinRequestResponse);
		System.out.println("** ObjectId after insert = " + sinRequestResponse.getId());
	...
	public static void main(String[] args) throws Exception {
		...
		WebResource webResource = client.resource("http://"
				+ "localhost:8081" + "/cs/v1/getCustomer/"
				+ "123");
